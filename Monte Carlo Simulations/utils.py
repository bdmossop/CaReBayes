...
def train_regime_models_bayesian(X:pd.DataFrame, 
    ...
    for regime, formulas in all_formulas.items():
        ...
        for target, equation in formulas.items():
            ...
            idata = model.fit(draws=1000, tune=2000, chains=4, inference_method='mcmc', progressbar=False, target_accept=0.95, seed=0)
    ...
    for i in range(n_samples):
        ...
        for regime, formulas in all_formulas.items():
            ...
            for target in formulas.keys():
                ...
                inference_data = model.predict(idata=idata, data=sample, kind="response", inplace=False)
